...
function entryRoutes() {
  const authController = wrapAsyncController(
    new AuthController(new AuthService(new UserRepository(User)))
  );
  const projectController = wrapAsyncController(
    new ProjectController(
      new ProjectService(new ProjectRepository(Project), new UserRepository(User))
    )
  );
  const taskController = wrapAsyncController(
    new TaskController(new TaskService(new TaskRepository(Task)))
  );
  const router = Router();

  router.use('/project/task', authenticate, taskRoutes(taskController));
